...
namespace QuestingBots.Components.Spawning
{
    public class PMCGenerator : BotGenerator
    {
        ...
        protected override async Task<Models.BotSpawnInfo> GenerateBotGroupTask()
        {
            ...
            {
                groupSizeFactor = Singleton<ConfigUtil>.Instance.CurrentConfig.BotSpawns.PMCs.FractionOfMaxPlayersVsRaidET.InterpolateForFirstCol(RaidHelpers.GetRaidTimeRemainingFraction());
            }
        ...
        protected override IEnumerable<Vector3> GetSpawnPositionsForBotGroup(Models.BotSpawnInfo botGroup)
        {
            ...
            {
                Singleton<LoggingUtil>.Instance.LogWarning("Could not find a spawn point for PMC group");
            ...
            float minSpawnDistance = GetMinSpawnDistanceFromOtherPlayers(Singleton<ConfigUtil>.Instance.CurrentConfig.BotSpawns.PMCs);
            if (locationData.AreAnyPositionsCloseToOtherPlayers(spawnPositionsForGroup, minSpawnDistance, out float distance))
            {
