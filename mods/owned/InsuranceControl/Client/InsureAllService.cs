...
/// <summary>
/// Collects equippable player gear and insures it via vanilla <see cref="InsuranceCompanyClass"/>.
...
internal static class InsureAllService
{
        ...
        Action? onFinished = null)
    {
        ...
        var insurerId = InsuranceControlPlugin.GetSelectedInsurerId();
        var insurerName = InsuranceControlPlugin.GetSelectedInsurerDisplayName();
        var insurer = insurance.Insurers?.FirstOrDefault(t =>
            string.Equals(t.Id, insurerId, StringComparison.OrdinalIgnoreCase));
        if (insurer == null)
        {
            Notify($"Застраховать все: {insurerName} недоступен как страховщик.", true);
            onFinished?.Invoke();
            return;
        }
        ...
        _busy = true;
        if (InsuranceControlPlugin.Debug.Value)
        {
            InsuranceControlPlugin.Log.LogInfo(
                $"[InsuranceControl] Pricing {insurable.Count} item(s) with {insurerName} ({insurer.Id}).");
        }
        ...
        try
        {
            insurance.GetInsurePriceAsync(insurable, _ =>
            {
                try
                {
                    int price = SumPrice(insurance, insurer.Id, insurable);
                    ...
                    insurance.SelectedInsurerId = insurer.Id;
                    insurance.InsureItems(insurable, result =>
                    {
                        try
                        {
                            if (result.Failed)
                            {
                                ...
                                InsuranceControlPlugin.Log.LogWarning(
                                    $"[InsuranceControl] InsureItems failed: {result.Error}");
                            }
                            else
                            {
                                Notify(
                                    $"Застраховано у {insurerName}: {insurable.Count} шт. ({price:N0} ₽).",
                                    false);
                                if (InsuranceControlPlugin.Debug.Value)
                                {
                                    InsuranceControlPlugin.Log.LogInfo(
                                        $"[InsuranceControl] Insured {insurable.Count} item(s) for {price} RUB via {insurerName}.");
                                }
                            }
                        }
                        finally
                ...
                {
                    InsuranceControlPlugin.Log.LogError($"[InsuranceControl] Price callback error: {ex}");
        ...
        {
            InsuranceControlPlugin.Log.LogError($"[InsuranceControl] GetInsurePriceAsync error: {ex}");
            Notify("Страховка не удалась (см. лог BepInEx).", true);
            Finish(onFinished);
        }
    }
