...
- ForcedLoot table tightened to high-value barter, labs keycards, and top ammo only
- Teleport / AI alliance / curse now apply to **scavs only** (PMCs are no longer pulled in)
- F12: `SpawnCultists` (+ `CultistEscortCount`) — optional cultist priest + warrior squad on curse start
...
- On curse start (host/authority): teleport eligible scav AI into a ring near the player (NavMesh-snapped)
