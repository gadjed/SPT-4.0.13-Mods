...
internal static class AllyDownedTracker
{
    ...
    public static bool TryEnterDowned(Player player, EDamageType damageType)
    {
        ...
        if (!MedSuitePlugin.Defib.Enabled.Value)
        {
            return false;
        }
        ...
        var bleedout = MedSuitePlugin.Defib.BleedoutTime.Value;
    ...
    public static void OnRevived(Player player)
    {
        ...
        MedSuitePlugin.DefibDebug($"Cleared downed state for {player.ProfileId}");
    }
    ...
    private static void FinalizeBleedout(Player player)
    {
        try
        {
            ...
            MedSuitePlugin.DefibDebug($"Bleedout finished for {player.ProfileId}");
            hc.Kill(EDamageType.Undefined);
        }
        catch (System.Exception ex)
        {
            MedSuitePlugin.Log.LogError($"[MedSuite:Defib] Bleedout kill failed: {ex}");
        }
    }
