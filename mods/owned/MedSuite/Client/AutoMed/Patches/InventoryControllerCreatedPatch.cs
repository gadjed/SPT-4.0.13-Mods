...
internal class StashInventoryControllerCreatedPatch : ModulePatch
{
    protected override MethodBase GetTargetMethod()
    {
        ...
        if (type == null)
        {
            MedSuitePlugin.Log.LogError(
                "[MedSuite:AutoMed] Could not find stash InventoryController type (GClass3388)."
            );
        ...
        {
            MedSuitePlugin.Log.LogError(
                $"[MedSuite:AutoMed] Could not find ctor on {type.FullName}."
            );
            return typeof(object).GetMethod(nameof(ToString))!;
        }
