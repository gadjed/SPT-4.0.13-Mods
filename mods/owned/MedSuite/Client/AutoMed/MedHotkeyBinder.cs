...
namespace MedSuite.Client.AutoMed;
...
internal static class MedHotkeyBinder
{
    ...
    {
        if (!MedSuitePlugin.AutoMed.Enabled.Value
            || controller == null
        ...
        if (MedSuitePlugin.Instance != null)
        {
            // Two frames: move address / UI can still be mid-update on the first frame in stash.
            MedSuitePlugin.Instance.StartCoroutine(RefreshAfterFrames(2));
            return;
        }
    ...
    public static void Refresh(InventoryController controller)
    {
        ...
        {
            MedSuitePlugin.Log.LogError($"[MedSuite:AutoMed] Refresh failed: {ex}");
        }
    }
    ...
    )
    {
        ...
        if (bound != null && !MedSuitePlugin.AutoMed.OverwriteExisting.Value)
        {
        ...
        if (!result.Succeeded)
        {
            MedSuitePlugin.Log.LogWarning(
                $"[MedSuite:AutoMed] Failed to bind {candidate.ShortName} to {slot}: {result.Error}"
            );
            return;
        }
        ...
        {
            MedSuitePlugin.Log.LogWarning(
                $"[MedSuite:AutoMed] RaiseEvents after bind failed: {ex.Message}"
            );
        }
        ...
        {
            MedSuitePlugin.Log.LogWarning(
                $"[MedSuite:AutoMed] TryRunNetworkTransaction after bind failed: {ex.Message}"
            );
        }

        MedSuitePlugin.Log.LogInfo(
            $"[MedSuite:AutoMed] Bound {Describe(candidate)} -> {slot} ({label})."
        );
    }
    ...
    {
        if (MedSuitePlugin.AutoMed.Debug.Value)
        {
            MedSuitePlugin.Log.LogInfo($"[MedSuite:AutoMed] {message}");
        }
    }
}
