...
CLIENT_DLL="$ROOT/Build/BepInEx/plugins/MedSuite.Client.dll"
SERVER_DIR="$ROOT/Build/SPT/user/mods/MedSuite"

if [[ ! -f "$CLIENT_DLL" ]]; then
  echo "Missing client DLL. Build Client first." >&2
  exit 1
fi
if [[ ! -f "$SERVER_DIR/MedSuite.dll" ]]; then
...
mkdir -p "$STAGE/SPT/user/mods/MedSuite"

cp "$CLIENT_DLL" "$STAGE/BepInEx/plugins/"
cp -R "$SERVER_DIR/." "$STAGE/SPT/user/mods/MedSuite/"
